...
class wxpay extends wxpayService {
     ...
     * @return: {Bool} 转账成功还是失败
     */
    async getTransferInfo(partner_trade_no) {
        return new Promise(async (resolve, reject) => {
            try {
                let needParam = await this.getTransferInfoParam(partner_trade_no);
                needParam.sign = await this.keyDigest(needParam, this.key.key);
                //返回的状态有三种 { SUCCESS:转账成功, FAILED:转账失败, PROCESSING:处理中}
                let status = await this.getTransferInfoPost(needParam);
                resolve(status)
            } catch (err) {
                reject(err)
            }
