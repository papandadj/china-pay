...
class alipayService {
    ...
    async appReduceParams( biz_content) {
        ...
        obj.timestamp = sd.format(new Date(), 'YYYYMMDDhhmmss');
     ...
     */
    async getSign(preStr, key) {
        let keyStr = key.privateKey.toString();
     ...
     * @return {Bool} 
     */
    async verifySignBase(params, key) {
        try {
            let publicKey = publicPem.toString();
            let preStr = this.getVerifyParamsCallback(params);
            let sign = params['sign'] ? params['sign'] : "";
            let verify = crypto.createVerify('RSA-SHA256');
            verify.update(preStr);
            return verify.verify(key.publicKey, sign, 'base64')
