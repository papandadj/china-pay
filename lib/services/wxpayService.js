...
class wxpayService {
    ...
    */
    ...
    //获取企业查询参数
    async getTransferInfoParam(partner_trade_no){
        let returnObj = {};
        returnObj.nonce_str = await this.createNonceStr();
        returnObj.partner_trade_no = partner_trade_no;
        returnObj.appid = this.wxpayJson.appid;
        returnObj.mch_id = this.wxpayJson.mch_id;
        return returnObj
    }

    async getTransferInfoPost(needParam){
        return new Promise(async (resolve, reject)=>{
            try {
                let formData = await this.obj2xml(needParam);
                let { response, body } = await this.fetchXml({ 
