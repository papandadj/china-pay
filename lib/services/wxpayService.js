...
class wxpayService {
    ...
    async transfersBase(obj) {
        return new Promise(async (resolve, reject) => {
            obj.mchid = obj.mch_id;
            obj.mch_appid = obj.appid;
            delete (obj.mch_id)
            delete (obj.appid)
            ...
            let key = obj.key;
            delete obj.key;
            obj.sign = await this.keyDigest(obj, key);
            let formData = await this.obj2xml(obj);
            try {
                let { response, body } = await this.fetchXml({
                    url: wxConfigJson.transferUrl, method: 'POST', body: formData, agentOptions: {
                        pfx: fs.readFileSync(wxConfigJson.pfxPath), //微信商户平台证书,
                        passphrase: obj.mchid
                    }
                });
                if (response.statusCode == 200) {
                    let bodyObj = await this.xml2obj(body);
